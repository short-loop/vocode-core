...
from vocode.streaming.models.telephony import PhoneCallDirection, PlivoConfig
from vocode.streaming.models.transcriber import TranscriberConfig
from vocode.streaming.output_device.plivo_output_device import (
...
from vocode.streaming.utils.state_manager import PlivoPhoneConversationStateManager


class PlivoPhoneConversationWebsocketAction(Enum):
    CLOSE_WEBSOCKET = 1


class PlivoPhoneConversation(AbstractPhoneConversation[PlivoOutputDevice]):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            output_device=PlivoOutputDevice(),
        ...
        self.plivo_config = plivo_config or PlivoConfig(
            account_sid=os.environ["PLIVO_ACCOUNT_SID"],
            auth_token=os.environ["PLIVO_AUTH_TOKEN"],
        )
        self.telephony_client = PlivoClient(
            base_url=self.base_url, maybe_plivo_config=self.plivo_config
        )
        self.plivo_sid = plivo_sid
        self.record_call = record_call

    def create_state_manager(self) -> PlivoPhoneConversationStateManager:
        return PlivoPhoneConversationStateManager(self)

    async def attach_ws_and_start(self, ws: WebSocket):
        super().attach_ws(ws)

        await self._wait_for_plivo_start(ws)
        await self.start()
        ...
        while self.is_active():
            ...
            if response == PlivoPhoneConversationWebsocketAction.CLOSE_WEBSOCKET:
                break
        await ws.close(code=1000, reason=None)
        await self.terminate()

    async def _wait_for_plivo_start(self, ws: WebSocket):
        assert isinstance(self.output_device, PlivoOutputDevice)
        while True:
            message = await ws.receive_text()
            if not message:
                continue
            ...
            if data["event"] == "start":
                ...
                self.output_device.stream_sid = data["start"]["streamId"]
                break

    async def _handle_ws_message(self, message) -> Optional[PlivoPhoneConversationWebsocketAction]:
        if message is None:
            return PlivoPhoneConversationWebsocketAction.CLOSE_WEBSOCKET
